...
errorH2O = 0.20 ; % Analytical uncertainty in H2O measurement [wt. %]
...
for idpdt = 1:length(dPdt)
    ...
    for ipress = 1:length(P_f)
        ...
        % Call the 1D diffusion function (Diffusion_function_1D_constant.m)
        [nodes, H2O_array, CO2_array] = Diffusion_function_1D_constant(P_i,P_f(ipress),lengthi,radius(ir),H2O_i,rho,TC,dPdt(idpdt));
...
[nodes, H2O_array, CO2_array] = Diffusion_function_1D_constant(P_i,P_f(IPF),lengthi,radius(IR),H2O_i,rho,TC,dPdt(IDPDT));
